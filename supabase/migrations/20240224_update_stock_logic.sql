-- DERMAKOR ACADEMY - FIX STOCK LOGIC
-- Rule: create_order_secure now only validates stock, doesn't decrement.
-- Payment flow (webhook or admin) handles the actual subtraction.
...
BEGIN
    ...
    FOR v_item IN SELECT * FROM jsonb_to_recordset(p_items) AS x(variant_id UUID, qty INTEGER) LOOP
        ...
        BEGIN
            ...
            -- REMOVED: Stock decrement from here.
            -- Logic: Stock is only deducted when PAID or confirmed.
        END;
    END LOOP;
