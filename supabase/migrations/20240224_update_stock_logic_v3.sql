-- DERMAKOR ACADEMY - STOCK LOGIC UPDATE
-- RULE: create_order_secure validates stock availability but DOES NOT decrement it.
-- Subtraction happens ONLY on payment confirmation (Webhook or Admin)
...
BEGIN
    ...
    FOR v_item IN SELECT * FROM jsonb_to_recordset(p_items) AS x(variant_id UUID, qty INTEGER) LOOP
        ...
        BEGIN
            ...
            -- [REMOVED] UPDATE public.product_variants SET stock_count = stock_count - v_item.qty WHERE id = v_item.variant_id;
        END;
    END LOOP;
